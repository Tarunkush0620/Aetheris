/**
 * Vite configuration for AETHERIS — a cinematic geospatial app.
 *
...
// Bypass strict SSL for older government API endpoints
...
import { spaceIntelligenceProxies } from './viteSpaceProxies.js';
...
const RADIO_USER_AGENT = 'Aetheris/1.0 (Radio Browser directory client)';
...
function celestrakProxy() {
  ...
  async function fetchUpstream(group) {
    ...
    const res = await fetch(url.toString(), {
      ...
      headers: { 'User-Agent': 'aetheris-celestrak-proxy/1.0 (+https://github.com/Tarunkush0620/Aetheris)' },
    });
    ...
    try {
      const upstream = await fetch(endpoint, {
        ...
        headers: {
          ...
          'User-Agent': 'aetheris-overpass-proxy/1.0',
        },
        body,
        signal: controller.signal,
      });
        ...
        try {
          ...
          try {
            const upstreamRes = await fetch(upstream, {
              signal: controller.signal,
              headers: { 'User-Agent': 'aetheris/dev (local)' },
            });
    ...
    try {
      const upstream = await fetch(
        `https://api.adsb.lol/v2/lat/${roundedLat}/lon/${roundedLon}/dist/${ADSBLOL_POINT_RADIUS_NM}`,
        {
          headers: {
            Accept: 'application/json',
            'User-Agent': 'aetheris-adsblol-regional-fallback/1.0',
          },
          signal: controller.signal,
        },
      );
        ...
        try {
          ...
          try {
            upstream = await fetch(upstreamUrl.toString(), {
              ...
              headers: {
                Accept: 'application/json',
                'User-Agent': 'aetheris-gbfs-proxy/1.0',
              },
              signal: controller.signal,
            });
          } finally {
...
const DEFAULT_CCTV_SOURCE_FILE = 'config/cctv_sources.global.json';
...
const DEFAULT_CCTV_MAX_SOURCES = 20000;
...
async function loadOhioSourcesFromOpenData() {
  const apiKey = process.env.OHGO_API_KEY;
  if (!apiKey) {
    console.warn('[CCTV] OHGO_API_KEY not set. Using synthetic Ohio cameras.');
    return [
      { id: 'ohio-1', name: 'I-71 at Broad St', city: 'Columbus', provider: 'OHGO', lat: 39.9612, lon: -82.9988, url: 'synthetic', feedType: 'image', sourceKind: 'global-cctv' },
      { id: 'ohio-2', name: 'I-75 at Hopple St', city: 'Cincinnati', provider: 'OHGO', lat: 39.1350, lon: -84.5385, url: 'synthetic', feedType: 'image', sourceKind: 'global-cctv' }
    ];
  }
  // If we had a real key, we'd fetch https://publicapi.ohgo.com/api/v1/cameras
  return [];
}

async function loadNztaSourcesFromOpenData() {
  const apiKey = process.env.NZTA_API_KEY;
  if (!apiKey) {
    console.warn('[CCTV] NZTA_API_KEY not set. Using synthetic NZTA cameras.');
    return [
      { id: 'nzta-1', name: 'SH1 Wellington Urban Motorway', city: 'Wellington', provider: 'NZTA', lat: -41.2865, lon: 174.7762, url: 'synthetic', feedType: 'image', sourceKind: 'global-cctv' },
      { id: 'nzta-2', name: 'SH1 Auckland Harbour Bridge', city: 'Auckland', provider: 'NZTA', lat: -36.8306, lon: 174.7461, url: 'synthetic', feedType: 'image', sourceKind: 'global-cctv' }
    ];
  }
  return [];
}

async function loadTaiwanSourcesFromOpenData() {
  const clientId = process.env.TDX_CLIENT_ID;
  const clientSecret = process.env.TDX_CLIENT_SECRET;
  if (!clientId || !clientSecret) {
    console.warn('[CCTV] TDX credentials not set. Using synthetic Taiwan cameras.');
    return [
      { id: 'taiwan-1', name: 'National Freeway 1 (Taipei)', city: 'Taipei', provider: 'Taiwan TDX', lat: 25.0330, lon: 121.5654, url: 'synthetic', feedType: 'image', sourceKind: 'global-cctv' },
      { id: 'taiwan-2', name: 'National Freeway 3 (Taichung)', city: 'Taichung', provider: 'Taiwan TDX', lat: 24.1477, lon: 120.6736, url: 'synthetic', feedType: 'image', sourceKind: 'global-cctv' }
    ];
  }
  return [];
}
...
async function refreshCctvSources() {
  ...
  if (needsLiveSources) {
    const [austinResult, caltransResult, tflResult, ohioResult, nztaResult, taiwanResult] = await Promise.allSettled([
      loadAustinSourcesFromOpenData(),
      loadCaltransSourcesFromOpenData(),
      tflEnabled ? loadTflSourcesFromOpenData() : Promise.resolve([]),
      loadOhioSourcesFromOpenData(),
    ...
    fromOhio = ohioResult.status === 'fulfilled' ? ohioResult.value : [];
    fromNzta = nztaResult.status === 'fulfilled' ? nztaResult.value : [];
    fromTaiwan = taiwanResult.status === 'fulfilled' ? taiwanResult.value : [];
  }
  // Live sources first so file/env overrides win on duplicate IDs (Map last-write).
  const merged = [...fromAustin, ...fromCaltrans, ...fromTfl, ...fromOhio, ...fromNzta, ...fromTaiwan, ...fromFile, ...fromEnv];
...
export async function fetchCctvImageFromUpstream(url, {
  ...
  try {
    const upstream = await fetchImpl(url, {
      headers: { 'User-Agent': 'aetheris-cctv-proxy/1.0' },
      signal: controller.signal,
    });
      ...
      server.middlewares.use('/api/cctv', async (req, res) => {
        try {
          ...
          if (url.pathname.startsWith('/media/')) {
            ...
            try {
              const upstreamHeaders = { 'User-Agent': 'aetheris-cctv-proxy/1.0' };
      ...
      server.middlewares.use('/api/adsblol/mil', async (req, res) => {
        try {
          ...
          const upstream = await fetch('https://api.adsb.lol/v2/mil', {
            headers: { 'User-Agent': 'aetheris-adsblol-proxy/1.0' },
          });
          const body = await upstream.text();
          if (upstream.ok) {
            _cache = body;
      ...
      try {
        ...
        const response = await fetch('https://api.openai.com/v1/responses', {
          ...
          body: JSON.stringify({
            ...
            instructions: [
              "Write one concise intelligence-HUD summary for AETHERIS.",
    ...
    middlewares.use('/api/realtime/token', async (req, res) => {
      ...
      const sessionConfig = {
        session: {
          ...
          instructions: [
            "You are GEV Voice Control, a concise voice controller for a Cesium geospatial app called AETHERIS.",
...
const GEV_REALTIME_TOOLS = [
  {
    ...
    description: "Fly the AETHERIS camera to a known city, geocoded country/region/city/landmark, or explicit WGS84 coordinate. Countries/cities frame the whole place; landmarks/buildings use close framing.",
    parameters: {
      type: 'object',
  ...
  },
  {
    ...
    description: "Enable or disable one registered AETHERIS data layer.",
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description: "Set the active AETHERIS visual filter/style.",
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
...
function fetchRegionalPlace(point) {
  const task = _nominatimQueue.then(async () => {
    ...
    const payload = await fetchRegionalJson(`https://nominatim.openstreetmap.org/reverse?${params}`, {
      headers: {
        'User-Agent': 'Aetheris/0.1 (+https://github.com/Tarunkush0620/Aetheris)',
        Referer: 'https://github.com/Tarunkush0620/Aetheris',
      },
    });
...
async function fetchRegionalNews(place) {
  ...
  const newsApiKey = String(process.env.NEWS_API_KEY || '').trim();
  const guardianApiKey = String(process.env.GUARDIAN_API_KEY || '').trim();

  if (newsApiKey) {
    try {
      const payload = await fetchRegionalJson(`https://newsapi.org/v2/everything?q=${encodedQuery}&sortBy=relevancy&pageSize=5`, {
        headers: { 'X-Api-Key': newsApiKey, 'User-Agent': 'Aetheris/0.1' },
        timeoutMs: 12_000,
      });
      if (payload.status === 'ok' && Array.isArray(payload.articles)) {
        const articles = payload.articles.slice(0, 5).map(a => ({
          title: a.title,
          url: a.url,
          source: a.source?.name || 'NewsAPI',
          timestamp: new Date(a.publishedAt).getTime()
        }));
        if (articles.length) return { status: 'ready', query, articles, source: 'NewsAPI' };
      }
    } catch (e) {
  ...
  if (guardianApiKey) {
    try {
      const payload = await fetchRegionalJson(`https://content.guardianapis.com/search?q=${encodedQuery}&api-key=${guardianApiKey}&page-size=5&order-by=relevance`, {
        headers: { 'User-Agent': 'Aetheris/0.1' },
        timeoutMs: 12_000,
      });
      if (payload.response && Array.isArray(payload.response.results)) {
        const articles = payload.response.results.map(a => ({
          title: a.webTitle,
          url: a.webUrl,
          source: 'The Guardian',
          timestamp: new Date(a.webPublicationDate).getTime()
        }));
        if (articles.length) return { status: 'ready', query, articles, source: 'The Guardian' };
      }
    } catch (e) {
      console.warn('[GuardianAPI Error]', e.message);
    }
  }
...
function redditProxy() {
  function install(middlewares) {
    middlewares.use('/api/reddit', async (req, res) => {
      const url = new URL(req.url || '', 'http://localhost');
      ...
      try {
        const fetchUrl = `https://old.reddit.com/search.json?q=${encodeURIComponent(q)}&sort=new&limit=25`;
        ...
        if (!redditRes.ok) {
          throw new Error(`Reddit API responded with status: ${redditRes.status}`);
        }
        ...
        console.warn(`[Reddit Proxy] Fetch failed, returning mock data. Error: ${err.message}`);
        // Return mock data for the query so the UI still works
        const mockData = {
          data: {
            children: [
              { data: { id: 'm1', title: `Just saw something crazy in ${q}!`, author: 'local_observer', subreddit_name_prefixed: `r/${q.replace(/\s+/g, '')}`, url: `https://reddit.com/r/${q.replace(/\s+/g, '')}` } },
              { data: { id: 'm2', title: `What is the weather going to be like in ${q} tomorrow?`, author: 'weather_fan', subreddit_name_prefixed: `r/${q.replace(/\s+/g, '')}`, url: `https://reddit.com/r/${q.replace(/\s+/g, '')}` } },
              { data: { id: 'm3', title: `Best places to eat around ${q}?`, author: 'foodie123', subreddit_name_prefixed: `r/${q.replace(/\s+/g, '')}`, url: `https://reddit.com/r/${q.replace(/\s+/g, '')}` } }
            ]
          }
        };
        res.writeHead(200, { 'Content-Type': 'application/json' });
...
function neoProxy() {
  function install(middlewares) {
    middlewares.use('/api/neo', async (req, res) => {
      const url = new URL(req.url || '', 'http://localhost');
      const start = url.searchParams.get('start_date') || new Date().toISOString().split('T')[0];
      const end = url.searchParams.get('end_date') || new Date().toISOString().split('T')[0];

      try {
        const apiKey = process.env.NASA_API_KEY;
        const neoUrl = `https://api.nasa.gov/neo/rest/v1/feed?start_date=${start}&end_date=${end}&api_key=${apiKey}`;
...
function openaqProxy() {
  const TTL_MS = 60 * 60_000;
  const CACHE_DIR = path.join(process.cwd(), '.gev-cache');
  const CACHE_PATH = path.join(CACHE_DIR, 'openaq.json');
  ...
  async function readDiskOnce() {
    ...
    try {
      const parsed = JSON.parse(await fsp.readFile(CACHE_PATH, 'utf8'));
      if (Number.isFinite(parsed?.at) && Array.isArray(parsed?.results)) {
        mem = parsed;
      }
    } catch { /* no disk cache yet */ }
  }
  ...
  async function writeDisk(entry) {
    try {
      await fsp.mkdir(CACHE_DIR, { recursive: true });
      const tempPath = CACHE_PATH + '.tmp' + Math.random().toString(36).slice(2);
      await fsp.writeFile(tempPath, JSON.stringify(entry), 'utf8');
      await fsp.rename(tempPath, CACHE_PATH);
    } catch (e) {
      console.error('[OpenAQ] Failed to write disk cache:', e.message);
    }
  }

  async function refresh() {
    // OpenAQ v2 is retired and v3 is returning invalid credentials.
    // Generating realistic global mock data for the Air Quality demonstration.
    const mockResults = [];
    const cities = [
      { lat: 40.71, lon: -74.00, pm25: 45 }, { lat: 34.05, lon: -118.24, pm25: 85 },
      { lat: 51.50, lon: -0.12, pm25: 35 }, { lat: 48.85, lon: 2.35, pm25: 50 },
      { lat: 35.67, lon: 139.65, pm25: 65 }, { lat: -33.86, lon: 151.20, pm25: 25 },
      { lat: 28.61, lon: 77.20, pm25: 350 }, { lat: 39.90, lon: 116.40, pm25: 210 },
      { lat: -23.55, lon: -46.63, pm25: 95 }, { lat: 1.35, lon: 103.81, pm25: 55 },
      { lat: 55.75, lon: 37.61, pm25: 40 }, { lat: -26.20, lon: 28.04, pm25: 75 },
      { lat: 19.43, lon: -99.13, pm25: 120 }, { lat: 37.77, lon: -122.41, pm25: 30 }
    ];

    // Add 150 random global points
    for (let i = 0; i < 150; i++) {
      cities.push({
        lat: (Math.random() * 140) - 70,
        lon: (Math.random() * 360) - 180,
        pm25: Math.random() * (Math.random() > 0.8 ? 200 : 80)
      });
    }
    ...
    for (const city of cities) {
      mockResults.push({
        ...
        parameters: [{ parameter: 'pm25', lastValue: city.pm25 }]
      });
    }

    const data = { results: mockResults };

    const entry = { at: Date.now(), results: data.results };
    mem = entry;
    writeDisk(entry);
    return entry;
  }
  ...
  return {
    name: 'gev-openaq-proxy',
    configureServer(server) {
      server.middlewares.use('/api/openaq', async (req, res) => {
        if (req.method !== 'GET') {
          res.statusCode = 405;
          return res.end();
        }
        ...
        try {
          ...
          if (mem && (now - mem.at) < TTL_MS) {
            res.setHeader('Content-Type', 'application/json');
            res.setHeader('Cache-Control', `public, max-age=${Math.floor((mem.at + TTL_MS - now) / 1000)}`);
            return res.end(JSON.stringify(mem));
          }
          ...
          if (mem) {
            console.warn('[OpenAQ] Refresh failed, serving stale cache');
            res.setHeader('Content-Type', 'application/json');
...
function fishingWatchProxy() {
  return {
    name: 'gev-fishing-watch-proxy',
    configureServer(server) {
      server.middlewares.use('/api/fishing', (req, res) => {
        if (req.method !== 'GET') {
          res.statusCode = 405;
          return res.end();
        }
        ...
        const mockVessels = [];
        const numVessels = 300;

        const zones = [
          { lat: 15, lon: 115, spread: 20 },
          { lat: 40, lon: -160, spread: 30 },
          { lat: -20, lon: -90, spread: 30 },
          { lat: 10, lon: -40, spread: 25 },
          { lat: 55, lon: -20, spread: 20 },
          { lat: -10, lon: 60, spread: 25 }
        ];

        for (let i = 0; i < numVessels; i++) {
          const zone = zones[Math.floor(Math.random() * zones.length)];
          const lat = zone.lat + (Math.random() - 0.5) * zone.spread;
          const lon = zone.lon + (Math.random() - 0.5) * zone.spread;

          const isDarkFleet = Math.random() < 0.15;

          mockVessels.push({
            id: `vessel-${Math.floor(Math.random() * 999999)}`,
            coordinates: { latitude: lat, longitude: lon },
            type: 'fishing',
            flag: isDarkFleet ? 'UNKNOWN' : ['CHN', 'JPN', 'USA', 'ESP', 'KOR', 'RUS'][Math.floor(Math.random() * 6)],
            lengthMeters: Math.floor(Math.random() * 50) + 15,
            tonnage: Math.floor(Math.random() * 500) + 50,
            speedKnots: (Math.random() * 12).toFixed(1),
            heading: Math.floor(Math.random() * 360),
            isDarkFleet: isDarkFleet,
        ...
        res.setHeader('Content-Type', 'application/json');
        res.setHeader('Cache-Control', 'public, max-age=60');
        return res.end(JSON.stringify({ vessels: mockVessels }));
      });
    }
  };
}
...
function amtrakerProxy() {
  return {
    name: 'gev-amtraker-proxy',
    configureServer(server) {
      server.middlewares.use('/api/trains', async (req, res) => {
        try {
          const upstream = await fetch('https://api.amtraker.com/v3/trains', {
            headers: { 'User-Agent': 'Aetheris/1.0' }
          });
          const text = await upstream.text();
          res.writeHead(upstream.status, {
            'Content-Type': 'application/json',
...
function mbtaProxy() {
  return {
    name: 'gev-mbta-proxy',
    configureServer(server) {
      server.middlewares.use('/api/mbta-vehicles', async (req, res) => {
        try {
          const upstream = await fetch('https://api-v3.mbta.com/vehicles', {
            headers: { 'User-Agent': 'Aetheris/1.0' }
          });
          const text = await upstream.text();
          res.writeHead(upstream.status, {
