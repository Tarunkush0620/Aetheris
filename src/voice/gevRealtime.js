...
// Voice cost control (repo-wide `Aetheris.<feature>.<field>` convention;
// the neighbouring ERROR_STORAGE_KEY predates it).
const VOICE_TIER_STORAGE_KEY = 'Aetheris.voiceCost.tier';
const VOICE_LIMITS_STORAGE_KEY = 'Aetheris.voiceCost.limits';
...
export function readStoredVoiceTier(storage) {
  try {
    const s = voiceStorage(storage);
    const raw = s?.getItem('Aetheris.voiceCost.tier') ?? s?.getItem('godsEyeView.voiceCost.tier');
...
export function writeStoredVoiceTier(tier, storage) {
  ...
  try {
    const s = voiceStorage(storage);
    s?.setItem('Aetheris.voiceCost.tier', resolved);
    s?.setItem('godsEyeView.voiceCost.tier', resolved);
  } catch {
    /* best effort */
  }
  return resolved;
...
export function readStoredVoiceLimits(storage) {
  try {
    const s = voiceStorage(storage);
    const raw = s?.getItem('Aetheris.voiceCost.limits') ?? s?.getItem('godsEyeView.voiceCost.limits');
...
export function writeStoredVoiceLimits(limits, storage) {
  ...
  try {
    const s = voiceStorage(storage);
    const serialized = JSON.stringify(serializeCostLimits(normalized));
    s?.setItem('Aetheris.voiceCost.limits', serialized);
    s?.setItem('godsEyeView.voiceCost.limits', serialized);
  } catch {
    /* best effort */
  }
  return normalized;
    ...
    const contextEvent = {
      ...
      item: {
        ...
        content: [
          {
            type: 'input_text',
            text: "Current AETHERIS viewport screenshot. Read any clearly visible street, building, and place labels in the image and combine them with the structured nearbyPlaces, streetLabels, and scene context. Do not invent labels that are not legible.",
          },
          {
            type: 'input_image',
            image_url: imageUrl,
...
async function captureViewportImage() {
  const viewer = window.__Aetheris?.viewer;
