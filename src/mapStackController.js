...
export class MapStackController {
  constructor(viewer, {
    ...
    this._imageryLayersArr = [];
    this._osmBuildingsTileset = null;
    ...
    // Dynamically show/hide OSM buildings based on camera pitch.
    // From directly above (pitch ~= -90°), the extruded boxes just cover
    // the satellite imagery and look terrible. We only show them when the
    // camera is tilted enough to see them in 3D perspective.
    this._osmBuildingsVisible = true;
    this.viewer.scene.preUpdate.addEventListener(() => {
      if (!this._osmBuildingsTileset) return;
      
      if (this.viewer.camera.pitch === undefined || this.viewer.scene.mode !== Cesium.SceneMode.SCENE3D) {
          if (this._osmBuildingsVisible) {
              this._osmBuildingsVisible = false;
              this._osmBuildingsTileset.show = false;
          }
          return;
      }
      
      const pitchDeg = Cesium.Math.toDegrees(this.viewer.camera.pitch);
      // pitch is negative: -90 = straight down, -45 = tilted, 0 = horizon
      // Show buildings when pitch is above -70° (i.e. tilted enough for 3D)
      const shouldShow = pitchDeg > -70;
      
      if (shouldShow !== this._osmBuildingsVisible) {
        this._osmBuildingsVisible = shouldShow;
        this._osmBuildingsTileset.show = shouldShow && this._activeId !== 'photoreal';
  ...
  async _activatePhotoreal(gen) {
    ...
    if (this._osmBuildingsTileset) this._osmBuildingsTileset.show = false;
  ...
  async _activateGlobeStack(stack, gen) {
    ...
    if (provider) {
      if (Array.isArray(provider)) {
        this._imageryLayersArr = provider.map(p => new Cesium.ImageryLayer(p));
        for (let i = 0; i < this._imageryLayersArr.length; i++) {
          this.viewer.imageryLayers.add(this._imageryLayersArr[i], i);
        }
      } else {
        const layer = new Cesium.ImageryLayer(provider);
        this._imageryLayersArr = [layer];
        this.viewer.imageryLayers.add(layer, 0);
      }
      
      if (this._vectorDataSource) {
        this._vectorDataSource.show = false;
      }
    ...
    // Reset our visibility tracker to force the preUpdate pitch listener
    // to re-evaluate whether they should be shown on the next frame.
    this._osmBuildingsVisible = null;
  }
  ...
  async _getImageryProvider(stack) {
    ...
    if (stack.kind === 'ion') {
      ...
      if (stack.id.includes('bing')) {
        // Prevent "Zoom Level Not Supported" tiles when zooming in too far
        Object.defineProperty(provider, 'maximumLevel', {
          get: () => 17,
          configurable: true
        });
      }
      ...
      const urlBase = cartoKey 
        ? `https://{s}.basemaps.cartocdn.com/rastertiles/dark_nolabels/{z}/{x}/{y}.png?key=${cartoKey}`
        : 'https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}.png';
        
      const urlLabels = cartoKey
        ? `https://{s}.basemaps.cartocdn.com/rastertiles/dark_only_labels/{z}/{x}/{y}.png?key=${cartoKey}`
      ...
      provider = [baseProvider, labelProvider];
    } else {
  ...
  _removeImageryLayer() {
    if (!this._imageryLayersArr || this._imageryLayersArr.length === 0) return;
    this._imageryLayersArr.forEach(layer => {
