...
export async function searchAndFlyTo(viewer, query, options = {}) {
  ...
  // Nominatim fallback if Google Maps fails (e.g. referer restrictions)
  if (!result && data.status !== 'OK') {
    try {
      ...
      if (nomData && nomData.length > 0) {
        ...
        types = [nom.type || 'political'];
        if (nom.boundingbox) {
          viewport = {
            southwest: { lat: parseFloat(nom.boundingbox[0]), lng: parseFloat(nom.boundingbox[2]) },
            northeast: { lat: parseFloat(nom.boundingbox[1]), lng: parseFloat(nom.boundingbox[3]) }
          };
        }
      }
    } catch (e) {
  ...
  const recovered = await placesNearViewRecovery(viewer, query, (lat != null && lng != null) ? { lat, lon: lng } : null);
  if (recovered) {
    lat = recovered.lat;
    lng = recovered.lon;
