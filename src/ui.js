...
/**
 * Central UI orchestrator for the AETHERIS application.
 *
 * Responsibilities:
    ...
    for (const cohort of snapshot.cohorts) {
      ...
      for (const item of cohort.nearest) {
        ...
        contacts.push({
          ...
          detail: `${cohort.id === 'military' ? 'MILITARY FLIGHT' : 'COMMERCIAL FLIGHT'} · ${Number.isFinite(item.distanceM)
...
export class StyleManager {
  ...
  _maybeNotifyLayoutReset() {
    try {
      const marker = `Aetheris.${PANEL_POSITION_STORAGE_VERSION}.layoutResetNotified`;
      if (localStorage.getItem(marker)) return;
      localStorage.setItem(marker, '1');
      const hadOldPositions = Object.keys(localStorage)
        .some((key) => key.startsWith('Aetheris.v6.panelPos.'));
      if (hadOldPositions) {
  ...
  _panelStorageKey(panelId) {
    return `Aetheris.${PANEL_POSITION_STORAGE_VERSION}.panelPos.${panelId}`;
  }
  ...
  _panelCollapseStorageKey(panelId) {
    return `Aetheris.${PANEL_LAYOUT_STORAGE_VERSION}.panelCollapsed.${panelId}`;
  }
  ...
  setRecordingMode(enabled, options = {}) {
    ...
    if (this._hudBtn) this._hudBtn.classList.toggle('active', this.hud.visible);
    this._syncShareState();
  }
