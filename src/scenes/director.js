...
const STORAGE_KEY = 'Aetheris.sceneProject.v2';
...
export class SceneDirector {
  ...
  constructor(viewer, styleManager, dataManager) {
    ...
    this._sceneLoopBtn = document.getElementById('scene-loop-btn');
    this._sceneProgress = document.getElementById('scene-progress');
  ...
  _initUI() {
    ...
    this._sceneLoopBtn?.addEventListener('click', () => {
      this._loopMode = !this._loopMode;
      this._sceneLoopBtn.classList.toggle('active', this._loopMode);
      this._sceneLoopBtn.setAttribute('aria-pressed', String(this._loopMode));
      this._updateStatus(this._loopMode ? 'Looping enabled' : 'Looping disabled');
    });

    this._sceneProgress?.addEventListener('click', (e) => {
  ...
  /** Reorder a shot up or down in the sequence. */
  moveShot(sceneId, shotId, direction) {
    const scene = this._project.scenes.find((item) => item.id === sceneId);
    if (!scene || !scene.shots) return;
    const idx = scene.shots.findIndex((item) => item.id === shotId);
    if (idx < 0) return;
    const targetIdx = idx + direction;
    if (targetIdx < 0 || targetIdx >= scene.shots.length) return;

    const [removed] = scene.shots.splice(idx, 1);
    scene.shots.splice(targetIdx, 0, removed);
    this._selectedShotId = removed.id;
    this._saveProject();
    this._renderShotList();
  }
  ...
  _renderShotList() {
    ...
    scene.shots.forEach((shot, idx) => {
      ...
      leftGroup.className = 'scene-shot-left-group';

      const badge = document.createElement('span');
      badge.className = 'scene-shot-badge';
      badge.textContent = String(idx + 1).padStart(2, '0');

      const reorderGroup = document.createElement('div');
      reorderGroup.className = 'scene-reorder-group';

      const upBtn = document.createElement('button');
      upBtn.className = 'scene-reorder-btn';
      upBtn.type = 'button';
      upBtn.title = 'Move shot up';
      upBtn.disabled = idx === 0;
      upBtn.innerHTML = '▲';
      upBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        this.moveShot(scene.id, shot.id, -1);
      });

      const downBtn = document.createElement('button');
      downBtn.className = 'scene-reorder-btn';
      downBtn.type = 'button';
      downBtn.title = 'Move shot down';
      downBtn.disabled = idx === scene.shots.length - 1;
      downBtn.innerHTML = '▼';
      ...
      leftGroup.appendChild(reorderGroup);
      ...
      loadBtn.className = 'scene-shot-btn scene-shot-load-btn';
      loadBtn.type = 'button';
      loadBtn.textContent = 'LOAD';
      loadBtn.title = 'Fly camera to this shot';
      ...
      stylePill.className = 'scene-tag-pill scene-tag-style';
      stylePill.textContent = style.toUpperCase();

      const modePill = document.createElement('span');
      modePill.className = 'scene-tag-pill scene-tag-mode';
      modePill.textContent = mode;

      const timePill = document.createElement('span');
      timePill.className = 'scene-tag-pill scene-tag-time';
      timePill.textContent = `⏱ ${(shot.durationSec || 5).toFixed(1)}s + ${(shot.holdSec || 1).toFixed(1)}s`;

      meta.appendChild(stylePill);
  ...
  _startProgressTicker(totalSec) {
    ...
    this._progressTimer = setInterval(() => {
      ...
      const progress = (elapsedMs % totalMs) / totalMs;
      const elapsedSec = (elapsedMs / 1000) % totalSec;
      this._setProgress(progress, elapsedSec, totalSec);
    }, 100);
  }
   ...
   */
  _setProgress(progress, elapsedSec = null, totalSec = null) {
    if (this._sceneProgressFill) {
