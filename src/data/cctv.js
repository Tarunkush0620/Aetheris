 ...
 * @module cctv
 *
 * CCTV camera data layer for AETHERIS.
 *
 * Architecture:
...
export const CCTV_CALIBRATION_STORAGE_KEY_V1 = 'Aetheris.cctv.calibration.v1';
/** v2 store key. Entries: { values: <7-field calibration offsets>, source: 'manual', savedAt: <epoch ms> }. */
export const CCTV_CALIBRATION_STORAGE_KEY_V2 = 'Aetheris.cctv.calibration.v2';
...
const cctvLayer = {
  ...
  async init(viewer) {
    ...
    const badge = document.getElementById('cctv-count-badge');
    if (badge) {
      badge.textContent = _count > 0 ? `${_count.toLocaleString()} CAMERAS` : '';
    }
