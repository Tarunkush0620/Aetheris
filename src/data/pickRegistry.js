...
export function resolvePickId(picked) {
  ...
  const unwrap = (id) => {
    ...
    if (typeof id === 'object') {
      ...
      // Space intelligence layer picks: { layer: 'space-track', noradId, name }
      if (typeof id.layer === 'string' && (typeof id.noradId === 'number' || typeof id.noradId === 'string')) {
        return `${id.layer}:${id.noradId}`;
      }
      return undefined;
    }
    return id;
